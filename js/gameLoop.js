...
function gameLoop() {
  ...
  if (inLooksmaxxingChallenge != 2) {
    ...
    clicks += Math.floor((multiplier) * (multiplier2) * (AutomaticRizzers) * Math.ceil(CountryClubs**(1+Cars/10)) * (1 + RiceWashers) * (1+(5*Number(listSum(LooksmaxxingChallengesCompleted)))/100) );
    updateVisuals();
  }
}